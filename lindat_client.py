...
def main():
    ...
    parser.add_argument("--prompt", default="Translate from {src} to {tgt}. Only print out the translation without any explanations. If the source should not be translated (e.g. it is a piece of code, technical term, filename, or a name of a company), simply copy it to the output. Do not try to interpret unusual inputs, if the input is not in {src}, do not translate it. If there are any markup tags (e.g. HTML), transfer all the markup tags from source into the appropriate places in the target. Source: {sentence} Translation: ",
