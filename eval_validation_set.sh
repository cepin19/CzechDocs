...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--model MODEL_NAME] [--base-url BASE_URL]"
            exit 1
            ;;
    esac
done

# Create model-specific directory name (sanitize model name)
MODEL_SUFFIX=$(echo "$MODEL" | sed 's/[^a-zA-Z0-9._-]/_/g')

VALID_DIR="splits/valid"
RESULTS_DIR="validation_results_${MODEL_SUFFIX}"
...
for doc_dir in ${VALID_DIR}/data/html/html_*; do
    ...
    echo "  → Running eval_all_approaches.sh (uk→cs) with model ${MODEL}..."
    
    # Capture output and move results
    if bash eval_all_approaches.sh "${uk_html}" "${cs_html}" "cs" "${MODEL}" "${MODEL_SUFFIX}" "${BASE_URL}" > "${out_dir}/eval.log" 2>&1; then
            echo "  ✓ Evaluation completed"
            
            # Move output files to organized directory
            # Check both default "out" and model-specific directory
            if [ -d "out_${MODEL_SUFFIX}" ]; then
                mv "out_${MODEL_SUFFIX}"/* "${out_dir}/" 2>/dev/null || true
            elif [ -d "out" ]; then
                mv out/* "${out_dir}/" 2>/dev/null || true
            fi
